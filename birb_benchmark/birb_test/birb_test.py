...
class BiRBTest(ABC):
    ...
    def fitModel(self, results_per_depth, valid_depths):
        ...
        for i, depth_results in enumerate(results_per_depth):
            ...
            # We skip low values
            if(mean <= 0):
                print("Warning: Mean less than 0")
            else:
        ...
        # Linear regresion
        A = np.vstack([log_valid_depths, np.ones_like(log_valid_depths)]).T 
        res = lsq_linear(A, log_mean_per_depth, bounds=([-np.inf, -np.inf], [0, 0])) 

        logP_fit, logA_fit = res.x
        A_fit = np.exp(logA_fit)
        p_fit = np.exp(logP_fit)

        mean_infidelity = ((4**self.qubits - 1) / 4**self.qubits) * (1 - p_fit)

        return A_fit, p_fit, mean_infidelity, mean_per_depth


    def plot(self, results_per_depth, valid_depths):

        """
        Given the results of the experiment per each depth, and the valid
        depths, plot the results and the fitting curve.
        ...
        A_fit, p_fit, mean_infidelity, mean_per_depth = self.fitModel(results_per_depth, valid_depths)
        ...
        plt.scatter(valid_depths, mean_per_depth, color=color, s=40, zorder=4)
        ...
        plt.plot(m_fit, f_fit, label='Infidelity: '+str(mean_infidelity), color=color)
        ...
        ax.set_title("Fit curve for " + self.backend_name + " with " +
            str(self.qubits) + " qubits")
